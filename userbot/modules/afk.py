...
async def set_afk(afk_e):
    ...
    if string:
        ...
        \n{LANG['REASON']}: `{string}`")
        await afk_e.client(UpdateProfileRequest(about='Sahibim Şuan #AFK @Herlockuserbot ✨')) #BUNU KULLANIPTA 
    else:
        await afk_e.edit(LANG['IM_AFK'])
        await afk_e.client(UpdateProfileRequest(about='Sahibim Şuan #AFK @Herlockuserbot ✨'))
    SON_GORULME = time()
    if BOTLOG:
