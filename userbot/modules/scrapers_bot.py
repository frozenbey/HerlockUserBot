# Copyright (C) 2021 HerlockUserBot
# thx to Sakir Bey
...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./HerlockSticker.png"):
                os.remove("./HerlockSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("HerlockSticker.png", "png")
            Resim = "HerlockSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' HerlockSticker.png")
            os.remove(sticker)
            Resim = "HerlockSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} HerlockThumb.jpg")
            os.remove(Resim)
            Resim = 'HerlockThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./herlockmeme.png"):
            os.remove("./herlockmeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./herlockmeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
